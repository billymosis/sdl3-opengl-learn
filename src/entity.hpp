...
struct SquareAABB : public BoundingVolume {
  ...
  bool isOnOrForwardPlane(const Plane &plane) const final {
    ...
    const float r =
        extent * (std::abs(plane.normal.x) + std::abs(plane.normal.y) +
  ...
  bool isOnFrustum(const Frustum &camFrustum,
    ...
    const SquareAABB globalAABB(globalCenter,
                                std::max(std::max(newIi, newIj), newIk));
...
struct AABB : public BoundingVolume {
  ...
  std::array<glm::vec3, 8> getVertice() const {
    ...
    vertice[0] = {center.x - extents.x, center.y - extents.y,
                  center.z - extents.z};
    vertice[1] = {center.x + extents.x, center.y - extents.y,
                  center.z - extents.z};
    vertice[2] = {center.x - extents.x, center.y + extents.y,
                  center.z - extents.z};
    vertice[3] = {center.x + extents.x, center.y + extents.y,
                  center.z - extents.z};
    vertice[4] = {center.x - extents.x, center.y - extents.y,
                  center.z + extents.z};
    vertice[5] = {center.x + extents.x, center.y - extents.y,
                  center.z + extents.z};
    vertice[6] = {center.x - extents.x, center.y + extents.y,
                  center.z + extents.z};
    vertice[7] = {center.x + extents.x, center.y + extents.y,
                  center.z + extents.z};
    return vertice;
  }
...
public:
  ...
  AABB getGlobalAABB() {
    // Get global scale thanks to our transform
    const glm::vec3 globalCenter{transform.getModelMatrix() *
                                 glm::vec4(boundingVolume->center, 1.f)};
    ...
    const glm::vec3 forward =
        transform.getForward() * boundingVolume->extents.z;
  ...
  // By default you can use the default constructor and don't put argument
  // input.
